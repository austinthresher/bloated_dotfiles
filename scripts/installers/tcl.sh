...
for TCL in tcl8.6.10 tk8.6.10; do
    ...
    cd "$TCL/unix" || exit 1
    ./configure --prefix="$HOME/.local" $CONFIG
    make
    make install
    CONFIG=--with-tcl="$TMP/tcl8.6.10/unix"
done

[ ! -e "$HOME/.local/bin/tclsh" ] && ln -s "$HOME/.local/bin/tclsh8.6" "$HOME/.local/bin/tclsh"
...
cp -r nagelfar131 "$HOME/.local/"
TARGET="$HOME/.local/bin/nagelfar"
rm -f "$TARGET"
ln -s "$HOME/.local/nagelfar131/nagelfar.tcl" "$TARGET"
